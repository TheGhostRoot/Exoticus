...
async def on_message_create(event: hikari.MessageCreateEvent) -> None:
    ...
    if guild_id:
        member = await plugin.bot.rest.fetch_member(guild_id, 1140708328046084266)
    if content and guild_id:
        if "discord.gg/" in content:
            ...
            if any(invite_code in content for invite_code in all_guild_invite_codes):
                pass
            else:
                await plugin.bot.rest.delete_message(event.message.channel_id, event.message.id)
                channel = await plugin.bot.rest.fetch_channel(event.message.channel_id)
                if isinstance(channel, hikari.TextableChannel):
                    message = await channel.send(f"{event.message.author.mention} Your message contained an invite and has been removed.")
                    # Delete the message after 5 seconds
                    await asyncio.sleep(5)
                    await plugin.bot.rest.delete_message(channel.id, message.id)
        ...
        #     query = (
        #         Query.from_(sector.system)
        #         .select(
        #             sector.system.forbidden_words,
        #         )
        #         .where(sector.system.guild_id == guild_id)
        #     )
        #     query_str = str(query)
        #     forbidden_words = await bot_obj.db.records(query_str)
        #     print(forbidden_words)

        # elif any(word in content for word in forbidden_words):
                    class DotDict(dict):
                    ...
                    event_attributes = DotDict({
                        "options": {
                            "remarks": "Automated Sanction",
                    ...
                    await bot_obj.moderation.mod_ban(event_attributes)
